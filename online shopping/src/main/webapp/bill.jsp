...
ResultSet rs2=st.executeQuery("select * from users0 inner join where cart.email='"+email+"'and cart.status='bill'");
     while(rs.next())
     {     
%>
...
<div class="right-div"><h3>Mobile Number:<%=rs2.getInt(20) %> </h3></div>  
...
<h3>Product Details</h3>
  ...
  <%
  ResultSet rs1=st.executeQuery("select* from product where cart.product_id=product.id and cart.email='"+email+"'and cart.status=''bill");
  while(rs1.next())
  {
	  sno=sno+1;
  %>
