...
try{
	...
	ResultSet rs=st.executeQuery("select*from product1 where id='"+product_id+"'");
	while(rs.next())
	{
	...
	ResultSet rs1=st.executeQuery("select*from cart where product_id="+product_id+" and email='"+email+"' and address is Null");
       ...
       if(z==1)
       {
    	   st.executeUpdate("update cart set total='"+cart_total+"',quantity='"+quantity+"',where product_id='"+product_id+"', email='"+email+"',and address is Null");
